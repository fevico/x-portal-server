...
export class InvoiceController {
    ...
    @Get(":invoiceId")
    @UseGuards(JwtAuthGuard)
    async getInvoiceById(@Request() req: RequestExpress, @Param('invoiceId') invoiceId: string): Promise<any> {
        const user = req.user as AuthenticatedUser;
    ...
    @Patch(":invoiceId")
    @UseGuards(JwtAuthGuard)
    async updateInvoice(@Request() req: RequestExpress, @Param('invoiceId') invoiceId: string, @Body() body: UpdateInvoiceDto): Promise<any> {
        const user = req.user as AuthenticatedUser;
    ...
    @Put(":invoiceId")
    @UseGuards(JwtAuthGuard)
    async updateDiscountInvoice(@Request() req: RequestExpress, @Param('invoiceId') invoiceId: string, @Body() body: InvoiceDto): Promise<any> {
        const user = req.user as AuthenticatedUser;
        const invoice = await this.invoiceService.getInvoiceById(invoiceId, user);
        if (!invoice) {
            throw new NotFoundException('Invoice not found');
