...
export class LessonPlanController {
  ...
  async getLessonPlanById(
    ...
    const user = req.user as AuthenticatedUser;
    return this.lessonPlanService.schoolLessonPlanById(
      user.schoolId,
      lessonPlanId,
    );
  }
  ...
  async updateLessonPlan(
    @Param('lessonPlan') lessonPlan: string,
    @Req() req: RequestExpress,
    @Body('status') status: LessonPlanType,
  ) {
    const user = req.user as AuthenticatedUser;
    return this.lessonPlanService.updateLessonPlanStatus(
      user.schoolId,
      lessonPlan,
      status,
    );
  ...
  @UseGuards(JwtAuthGuard)
  async fetchSessionTermWeeks(
    @Req() req: RequestExpress,
    @Param('sessionId') sessionId: string,
    @Param('termId') termId: string,
  ) {
    const school = req.user as AuthenticatedUser;
    return this.lessonPlanService.fetchSessionTermWeeks(
      school.schoolId,
      sessionId,
      termId,
    );
