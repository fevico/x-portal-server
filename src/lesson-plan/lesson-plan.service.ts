...
export class LessonPlanService {
    ...
    try {
      ...
      if (body.classArmId) {
        ...
        if (!classArm) {
          throw new NotFoundException(
            `ClassArm with ID ${body.classArmId} not found`,
          );
        }
      }
      ...
      const plan = await this.prisma.lessonPlan.create({
        data: {
          ...
          classArm: body.classArmId
            ? { connect: { id: body.classArmId } }
            : undefined,
      ...
      if (error.code === 'P2002') {
        throw new BadRequestException(
          'A lesson plan with these details already exists',
        );
      }
      if (error.code === 'P2025') {
        throw new NotFoundException(
          'One or more referenced records (school, term, session, subject, class, classArm, or week) not found',
        );
      }
      throw new BadRequestException(
        `Failed to create lesson plan: ${error.message}`,
      );
    }
  }
    ...
    try {
      ...
      if (!school) {
        throw new UnauthorizedException(
          'Unauthorized request: School not found',
        );
      }
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2025') {
          throw new NotFoundException(
            'No lesson plans found for the specified criteria',
          );
        }
  ...
  async schoolLessonPlanById(schoolId: string, lessonPlanId: string) {
    try {
      // Validate school existence
      const school = await this.prisma.school.findUnique({
        where: { id: schoolId },
      ...
      if (!lessonPlan) {
        throw new NotFoundException(
          'Lesson plan not found for the specified criteria',
        );
      }
  ...
  async fetchSessionTermWeeks(schoolId, sessionId, termId) {
    try {
      ...
      const termWeeks = await this.prisma.weeks.findMany({
