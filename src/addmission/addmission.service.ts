...
export class AddmissionService {
    ...
    async createAdmission(dto: CreateAdmissionDto, file?: Express.Multer.File): Promise<Admission> {
        // Validate school
        const school = await this.prisma.school.findUnique({ where: { id: dto.schoolId } });
        if (!school) {
          throw new NotFoundException('School not found');
        ...
        if (!guardian) {
          ...
          guardian.surname !== dto.guardianSurname ||
          guardian.middleName !== dto.guardianMiddleName ||
          guardian.firstName !== dto.guardianFirstName
        ) {
          throw new ConflictException('Guardian email already exists with different details');
        }
    
        // Create OtherInfo
        const otherInfo = await this.prisma.otherInfo.create({
          data: {
            healthProblem: dto.healthProblem,
        ...
        const schoolInformation = await this.prisma.schoolInformation.create({
          data: {
            schoolName: dto.previousSchoolName,
            address: dto.previousSchoolAddress,
            contact: dto.previousSchoolContact,
          },
        });
        ...
        return this.prisma.admission.create({
          data: {
            ...
            image: imageUrl,
            guardianId: guardian.id,
            otherInfoId: otherInfo.id,
            schoolInformationId: schoolInformation.id,
    ...
    async updateAdmission(id: string, dto: UpdateAdmissionDto, file?: Express.Multer.File): Promise<Admission> {
        const admission = await this.prisma.admission.findUnique({ where: { id } });
        if (!admission) {
          throw new NotFoundException('Admission not found');
        }
        ...
        if (dto.localGovernment) updateData.localGovernment = dto.localGovernment;
        if (dto.isDeleted !== undefined) updateData.isDeleted = dto.isDeleted;
        if (dto.isAccepted !== undefined) updateData.isAccepted = dto.isAccepted;
        ...
        if (dto.presentClassId) {
          ...
          updateData.presentClass = { connect: { id: dto.presentClassId } };
        }
        if (dto.classApplyingId) {
          const classApplying = await this.prisma.class.findUnique({ where: { id: dto.classApplyingId } });
          if (!classApplying || classApplying.schoolId !== admission.schoolId) {
          ...
          updateData.classApplying = { connect: { id: dto.classApplyingId } };
        }
        ...
        if (dto.guardianEmail) {
          ...
          if (!guardian) {
            guardian = await this.prisma.guardian.create({
              data: {
                surname: dto.guardianSurname || admission.surname,
                middleName: dto.guardianMiddleName,
                firstName: dto.guardianFirstName || admission.firstName,
                email: dto.guardianEmail,
                phone: dto.guardianPhone,
                address: dto.guardianAddress,
                relationship: dto.guardianRelationship,
            ...
            await this.prisma.guardian.update({
              ...
              data: {
                surname: dto.guardianSurname || guardian.surname,
                middleName: dto.guardianMiddleName,
                firstName: dto.guardianFirstName || guardian.firstName,
                phone: dto.guardianPhone,
                address: dto.guardianAddress,
                relationship: dto.guardianRelationship,
              },
          ...
          updateData.guardian = { connect: { id: guardian.id } };
        }
    
        // Update OtherInfo
        if (dto.healthProblem || dto.hearAboutUs) {
          await this.prisma.otherInfo.update({
            where: { id: admission.otherInfoId },
            data: {
              healthProblem: dto.healthProblem || (await this.prisma.otherInfo.findUnique({ where: { id: admission.otherInfoId } })).healthProblem,
              hearAboutUs: dto.hearAboutUs || (await this.prisma.otherInfo.findUnique({ where: { id: admission.otherInfoId } })).hearAboutUs,
            },
          });
        }
    
        // Update SchoolInformation
        if (dto.previousSchoolName || dto.previousSchoolAddress || dto.previousSchoolContact) {
          await this.prisma.schoolInformation.update({
            where: { id: admission.schoolInformationId },
            data: {
              schoolName: dto.previousSchoolName || (await this.prisma.schoolInformation.findUnique({ where: { id: admission.schoolInformationId } })).schoolName,
              address: dto.previousSchoolAddress || (await this.prisma.schoolInformation.findUnique({ where: { id: admission.schoolInformationId } })).address,
              contact: dto.previousSchoolContact || (await this.prisma.schoolInformation.findUnique({ where: { id: admission.schoolInformationId } })).contact,
            },
          });
        }
      ...
      async acceptAdmission(id: string): Promise<Admission> {
        const admission = await this.prisma.admission.findUnique({ where: { id } });
        if (!admission) {
          throw new NotFoundException('Admission not found');
        }
        ...
        if (admission.isAccepted) {
          throw new ConflictException('Admission already accepted');
        }
