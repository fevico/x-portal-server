...
async function main() {
    ...
    try {
        // Clean existing data first by directly deleting all ClassArm records with specific names
        ...
        // Get all school IDs to clean
        const schoolsToClean = await prisma.school.findMany({
            where: {
                name: {
                    in: schoolNames,
                },
        ...
        const schoolIds = schoolsToClean.map((s) => s.id);

        if (schoolIds.length > 0) {
            console.log(`🗑️ Found ${schoolIds.length} schools to clean`);

            // Delete in reverse order to respect foreign key constraints
            // Most dependent records first
            await prisma.studentScoreAssignment.deleteMany({
            ...
            console.log('ℹ️ No existing schools found to clean');
        }
        ...
        for (const [schoolIndex, schoolData] of schools.entries()) {
            ...
            // Create configuration for school
            await prisma.configuration.create({
                data: {
                    ...
                    schoolHeadName: `Principal ${generateNigerianName().lastName}`,
                    schoolHeadContact: generateNigerianPhone(),
                    principalName: `Principal ${generateNigerianName().lastName}`,
                    principalContact: generateNigerianPhone(),
                    bursarName: `Bursar ${generateNigerianName().lastName}`,
                    bursarContact: generateNigerianPhone(),
                    createdBy: 'system',
                },
            });

            console.log(`⚙️ Configuration created for ${school.name}`);
            ...
            // Create class arms (independent of classes - they can be assigned to any class)
            for (const armName of classArms) {
                const classArm = await prisma.classArm.upsert({
                    where: {
                        name_schoolId: {
                            name: armName,
                            schoolId: school.id,
            ...
            for (const classRecord of createdClasses) {
                // Assign all class arms to each class
                for (const classArm of createdClassArms) {
            ...
            for (const classRecord of createdClasses) {
                ...
                // Assign all class arms to each class since they're now school-scoped
                for (const classArm of createdClassArms) {
            ...
            for (let i = 0; i < studentsToCreate; i++) {
                ...
                // Pick a random class arm from all available arms (since they're school-scoped)
                const randomClassArm =
                    createdClassArms[Math.floor(Math.random() * createdClassArms.length)];
                ...
                // Determine if student should be alumni (10% chance)
                const isAlumni = Math.random() < 0.1;
            ...
            // Create staff (teachers) for the school
            const staffToCreate = Math.floor(Math.random() * 10) + 15; // 15-25 staff per school
            console.log(`👨‍🏫 Creating ${staffToCreate} teachers...`);
            ...
            const teacherQualifications = [
                ...
                'B.Ed, Social Studies',
                'B.A, History',
                'B.Th, Religious Studies',
                'B.Agric, Agricultural Science',
            ];

            for (let i = 0; i < staffToCreate; i++) {
                const staffName = generateNigerianName();
                ...
                const staffUser = await prisma.user.create({
                    data: {
                        ...
                        username: `teacher_${school.id.slice(-4)}_${i + 1}`,
                        email: `teacher${i + 1}_${school.id.slice(-4)}@example.com`,
                        contact: generateNigerianPhone(),
                        password: hashedPassword,
                        plainPassword: adminPassword,
                        role: 'admin',
                ...
                // Create staff record for the teacher
                await prisma.staff.create({
                    data: {
                        userId: staffUser.id,
                        staffRegNo: `${school.name
                            .substring(0, 3)
                            .toUpperCase()}/TCH/${String(i + 1).padStart(4, '0')}`,
                        // department:
                        //     teachingSubjects[
                        //     Math.floor(Math.random() * teachingSubjects.length)
                        //     ],
                        // position: 'Teacher',
                        hireDate: new Date(
                            2015 + Math.floor(Math.random() * 9), // Hired between 2015-2024
                            Math.floor(Math.random() * 12),
                            Math.floor(Math.random() * 28) + 1,
                        ),
                        qualifications:
                            teacherQualifications[
                            Math.floor(Math.random() * teacherQualifications.length)
                            ],
                        createdBy: adminUser.id,
                    },
                });
            ...
            console.log(`✅ Created ${staffToCreate} teachers`);
        ...
        console.log(`- Students, parents, teachers, and admissions created`);
        console.log(`- Some students marked as alumni (10% chance)`);
        ...
        console.log(
            'Also: students, parents, and teachers have similar credentials with pattern student_XXXX_N, parent_XXXX_N, and teacher_XXXX_N',
        );
    } catch (error) {
